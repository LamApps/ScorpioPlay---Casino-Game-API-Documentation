# 💻 Scorpio Play API - Seamless Integration Code Examples

This page provides **Seamless Integration Code Examples** written in **JavaScript (Node.js)** and **PHP**.  

These examples cover various **callback request types** and **response data structures**. Please review them carefully.

---

## ⚙️ Key Steps to Handle a Callback Request

1. **Create a route (callback URL)**  
   Define an endpoint on your server where the API will send callback requests.

2. **Parse the incoming request**  
   Read the JSON payload and extract the required data such as `command`, `data`, `timestamp`, and `check`.

3. **Respond with the appropriate data**  
   Send back the response in the **exact JSON structure** as required by SlotCity API.

> Implement all callback processing logic on your **server side**.  

---

## ⚠️ Important Notes

- These code examples are **for demonstration purposes only**.  
- **Do not use them in production** without proper validation and security measures.  
- Pay **close attention to the response JSON structure**. Many developers overlook this and encounter `CALLBACK_ERROR`.
